...
function Insert_Product_Catalog($atts) {
	...
	if (isset($_POST['Comparison_Product_ID'])) {
		$ProductString .= $HeaderBar;
		$ProductString .= UPCP_Product_Comparison( intval( $_POST['Comparison_Product_ID'] ), $omit_fields);
		return $ProductString;
	}
...
function Insert_Minimal_Products($atts) {
	...
	if ($product_ids != "") {
		...
		$Products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE Item_ID IN (%s)", $Products_String ) );
	}
	elseif ($catalogue_id != "") {
		$Item_IDs = "'999999',";
		$Category_IDs = "'999999',";
		...
		if (in_array("Category", $Extra_Elements)) {$ProductString .= "<div class='prod-category-container upcp-product-side-container'>\n<div class='upcp-side-title'>" . $Additional_Info_Category_Label . " </div>" . $Product->Category_Name . "</div>";}
			...
			if (in_array("CustomFields", $Extra_Elements)) {
				...
				foreach ($Fields as $Field) {
					$Value = $wpdb->get_row( $wpdb->prepare( "SELECT Meta_Value FROM $fields_meta_table_name WHERE Item_ID=%dand Field_ID=%d", $Product->Item_ID, $Field->Field_ID ) );
...
function UPCP_Get_Related_Products($Product, $Related_Type = "Auto") {
	...
	if ($Related_Type == "Manual") {
		...
		$Related_Products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE Item_ID IN ( %s )", $ID_String ) );
	}
...
function AddCustomFields($ProductID, $Layout) {
	...
	$Fields = $wpdb->get_results( $wpdb->prepare( "SELECT Field_ID, Field_Name, Field_Type FROM $fields_table_name WHERE Field_Displays=%s OR Field_Displays='both' ORDER BY Field_Sidebar_Order", $Layout ) );
	if (is_array($Fields)) {
