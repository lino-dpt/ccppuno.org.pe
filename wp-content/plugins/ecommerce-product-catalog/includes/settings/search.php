...
class ic_settings_search {
	...
	function placeholder() {
		if ( isset( $_GET[ 'tab' ] ) && $_GET[ 'tab' ] === 'help' ) {
			...
			$placeholder = __( 'Search settings', 'ecommerce-product-catalog' ) . ' - ' . __( "try 'attributes'", 'ecommerce-product-catalog' );
		}
		return $placeholder;
	}
	...
	function search_html() {
		...
		?>
		<form class="ic-settings-search<?php echo $class ?>" action="<?php echo admin_url( 'edit.php' ); ?>">
			<?php
			...
			<input type="submit" class="button-secondary" value="<?php _e( 'Search', 'ecommerce-product-catalog' ) ?>">
		</form>
		<?php
